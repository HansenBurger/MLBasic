...
class NormPredictor(Basic):
    def __init__(self, algo_class: any):
        super().__init__()
        self.__algo_type = algo_class
        self.__data_set = []
        self.__para_slt = {}
        self.__pred_rst = None
    ...
    def DataSetBuild(self, data_: DataFrame, col_label: str, split_size: .3):
        '''
        ...
        data_l = list(main_p.DataSplit(split_size))

        try:
            balance_p = BalanSMOTE(data_l[0], data_l[1])
            data_l[0], data_l[1] = balance_p.OverSample()
        except:
            balance_p = BalanRandOS(data_l[0], data_l[1])
            a, b = balance_p.OverSample()
            print(type(a), type(b))
            data_l[0], data_l[1] = balance_p.OverSample()

        self.__data_set = data_l
    ...
    def ParamSelectRand(self, para_pool: dict, eval_set: bool = False):
        ...
        main_p = ParaSel_Rand(self.__data_set, para_init)
        para_deduce = {
        ...
        self.__para_slt = main_p.BestParam()
    ...
    def Validate(self,
        ...
        para_init = self.__para_slt
        para_init.update(para_init_add)
        main_p = self.__algo_type(self.__data_set, para_init)
        ...
        self.__pred_rst = main_p.perform
    ...
    def ResultGenerate(self,
        ...
        '''
        '''
        main_p = OutcomeGenerate(self.__pred_rst, save_path)
